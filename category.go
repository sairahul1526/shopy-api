...
func CategoryGet(w http.ResponseWriter, r *http.Request) {
	...
	if ok {
		...
		if len(where) > 0 {
			count, _, _ := selectProcess("select count(*) as ctn from `" + categoryTable + "` where " + where)
			pagination["total_count"] = count[0]["ctn"]
		} else {
			count, _, _ := selectProcess("select count(*) as ctn from `" + categoryTable + "`")
			pagination["total_count"] = count[0]["ctn"]
		}
...
func CategoryAdd(w http.ResponseWriter, r *http.Request) {
	...
	fieldCheck := requiredFiledsCheck(body, categoryRequiredFields)
	if len(fieldCheck) > 0 {
	...
	var ok bool
	for true {
		body["category_u_id"] = RandStringBytes(categoryDigits)
		status, ok = insertSQL(categoryTable, body)
	...
	w.Header().Set("Status", status)
	if ok {
		response["category_u_id"] = body["category_u_id"]
		response["meta"] = setMeta(status, "Category Row added", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
...
func CategoryUpdate(w http.ResponseWriter, r *http.Request) {
	...
	for _, field := range categoryRequiredFields {
	...
	status, ok := updateSQL(categoryTable, r.URL.Query(), body)
	w.Header().Set("Status", status)
	if ok {
		response["meta"] = setMeta(status, "Category Row updated", dialogType)
	} else {
		response["meta"] = setMeta(status, "", dialogType)
	}
